...
- **計算ロジック**: `client/src/lib/brew-calculator.ts` に各メソッドの純粋関数（UI 非依存、`BrewRecipe` を返す）を集約。
  - `calculateRecipe`（**4:6**）: 総湯量（豆量 ×15）→ 前半 40%（味）を 2 投に分割 → 後半 60%（濃度）を均等割り →
    各投に 45 秒間隔の開始タイム・累計湯量・目安抽出時間を付与。
  - `calculateRecipe1010`（**10:10**）: 湯量（豆量 ×15）を 10 等分。蒸らし 45 秒＋以降 30 秒間隔で 10 投。
  - `calculateLatteRecipe`（**カフェラテ**）: 湯量の 1/10 の豆で 5 投（粉:湯 = 1:10）＋
    コーヒー:ミルク = 合計 10 パートの配分（`1:9`〜`10:0`、既定 `4:6`）からミルク量・できあがり量を算出。
    抽出湯量は固定で、比率を変えるとミルク量だけが増減する。
  - `calculateFlashRecipe`（**アイス / フラッシュブリュー**）: 出来上がり総量（豆量 ×13〜16、`FLASH_STRENGTHS` で切替）の
    40% を「氷」としてサーバーに先入れ、残り 60% の「お湯」を 4:6 構造で氷に落として急冷。豆:お湯:氷 ≒ 1:9:6（標準）。
  - メソッド一覧は `BREW_METHODS`、選択肢定義（`FLAVOR_BALANCES` / `STRENGTH_LEVELS` / `FLASH_STRENGTHS` / `ROAST_LEVELS`）も同ファイル。
